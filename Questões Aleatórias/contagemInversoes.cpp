...
int merge_sort(vector<int> &v){
	...
	// se não declaro os vetore u1 e u2 e faço cada um receber uma metade de v
	vector<int> u1, u2;
	...
	// ordeno u1 e u2 e adiciono a inv as inversões de cada metade do vetor e adiciono INF ao final de cada um deles
	inv+=merge_sort(u1);
	inv+=merge_sort(u2);
	u1.push_back(INF);
	u2.push_back(INF);
	...
	for(int i=0;i<v.size();i++){
		...
		if(u1[ini1]<=u2[ini2]){
			// então o coloco em v e incremento o valor de ini1
			v[i]=u1[ini1];
			ini1++;
			...
			v[i]=u2[ini2];
			ini2++;
			// não se esquecendo de adicionar o número de elementos em u1 ao total deinversões em v
			inv+=u1.size()-ini1-1;
		}
	}
